...
insert into literature (doc_title,doc_href,doc_filename) select title,url,path FROM spischolar.t_delivery GROUP BY title,url;
insert into help_record (help_email,doc_filename,literature_id) select t1.email,t1.path,t2.id from spischolar.t_delivery t1, literature t2 where t1.title = t2.doc_title and t1.url = t2.doc_href;